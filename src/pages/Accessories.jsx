  ...
  useEffect(() => {
    fetch("http://localhost:5000/api/products")
      ...
      .then(data => {
        ...
        const filtered = data.filter(product => product.category.toLowerCase() === "accessories");
    ...
    <div className="bg-softwhite">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16">
        {accessories.length === 0 ? (
          <p className="text-center text-gray-500">No accessories available yet.</p>
        ) : (
            ...
            {accessories.map(product => (
              <Link key={product._id} to={`/product/${product._id}`}>
                <div className="border p-4 rounded-lg shadow hover:shadow-xl hover:-translate-y-1 transition duration-300">
                  ...
                  />
                  <h2 className="text-lg mt-3 font-medium">{product.name}</h2>
